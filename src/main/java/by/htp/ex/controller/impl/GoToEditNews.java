...
public class GoToEditNews implements Command {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		News news;
		String id = request.getParameter(ControllerConstant.JSP_ID_PARAM);
		...
		try {
			...
			request.getSession().setAttribute(ControllerConstant.JSP_PRESENTATION_ATT,
					ControllerConstant.JSP_PRESENTATION_ATT_VALUE_EDIT_NEWS);
			request.getSession().setAttribute(ControllerConstant.JSP_INVALID_NEWS_DATA_ATT, null);
			request.getSession().setAttribute(ControllerConstant.JSP_EDIT_NEWS_SUCCESS_PARAM, null);
			request.getSession().setAttribute(ControllerConstant.JSP_EDIT_NEWS_STATUS_PARAM, true);
			request.getSession().setAttribute(ControllerConstant.JSP_ID_PARAM, id);
